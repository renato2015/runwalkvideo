...
public class CalendarSlotDao<T extends CalendarSlot<? super T>> extends JpaDao<T> {
	...
	public List<T> getFutureSlots() {
		 TypedQuery<T> query = createEntityManager().createQuery("SELECT calendarSlot FROM " + getTypeParameter().getSimpleName() + " WHERE calendarSlot.startDate <= :date", getTypeParameter())
