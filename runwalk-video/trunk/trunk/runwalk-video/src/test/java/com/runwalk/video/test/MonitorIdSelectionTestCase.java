...
public class MonitorIdSelectionTestCase extends TestCase {
	...
	public void testScreenIdCalculation() throws Exception {
		...
		for (int i = 2; i < 10; i++) {
			assertEquals(defaultMonitorId, WindowManager.getDefaultMonitorId(1, i));
		}
		
		//test for two attached screens, default screenId should be equal to the monitor free at that moment
		for (int i = 2; i < 10; i++) {
			assertEquals(freeMonitorId, WindowManager.getDefaultMonitorId(2, i));
		}

		//test for three attached screens, default screenId should alternate between 1 and 2
		for (int i = 2; i < 10; i++) {
			int expected = i % 2 == 0 ? 2 : 1;
			assertEquals(expected, WindowManager.getDefaultMonitorId(3, i));
		}
		
		//test for four attached screens, default screenId should alternate between 1, 2 and 3
		assertEquals(1, WindowManager.getDefaultMonitorId(4, 1));
		assertEquals(2, WindowManager.getDefaultMonitorId(4, 2));
		assertEquals(3, WindowManager.getDefaultMonitorId(4, 3));
		assertEquals(1, WindowManager.getDefaultMonitorId(4, 4));
		assertEquals(2, WindowManager.getDefaultMonitorId(4, 5));
		assertEquals(3, WindowManager.getDefaultMonitorId(4, 6));
		assertEquals(1, WindowManager.getDefaultMonitorId(4, 7));
